 ...
 */
class  Dec_candles_fragrancesview extends CI_Controller{
        ...
        public function index(){
            ...
            $config = array();
            $config['base_url'] = base_url().'dec_candles_fragrancesview/index';
            $config['total_rows'] = count($this->postad_kitchen_model->count_dcandles_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $kitchenhome_view = $this->postad_kitchen_model->dcandles_view($search_option);
                ...
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "dec_candles_fragrancesview",
                ...
                $data['busconcount'] = $this->postad_kitchen_model->busconcount_dcandles();
                 /*seller and needed count for kitchen*/
                $data['sellerneededcount'] = $this->postad_kitchen_model->sellerneeded_dcandles();
                 /*packages count*/
                $data['deals_pck'] = $this->postad_kitchen_model->deals_pck_dcandles();
         ...
         public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'dec_candles_fragrancesview/search_filters';
            $config['total_rows'] = count($this->postad_kitchen_model->count_dcandles_search());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             ...
             $rs = $this->postad_kitchen_model->dcandles_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['favourite_list']=$favourite_list;
             /*business and consumer count for kitchen*/
                $result['busconcount'] = $this->postad_kitchen_model->busconcount_dcandles();
                 /*seller and needed count for kitchen*/
                $result['sellerneededcount'] = $this->postad_kitchen_model->sellerneeded_dcandles();
                 /*packages count*/
                $result['deals_pck'] = $this->postad_kitchen_model->deals_pck_dcandles();
