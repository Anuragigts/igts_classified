 ...
 */
class  Laptop_computers_view extends CI_Controller{
        ...
        public function index(){
            $this->session->set_userdata('lappy_sub',array());
            ...
            $config = array();
            $config['base_url'] = base_url().'laptop_computers_view/index';
            $config['total_rows'] = count($this->classifed_model->count_lappy_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $smalls_result = $this->classifed_model->lappy_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                ...
                /*motor*/
                $data['lappy_sub'] = $this->hotdealsearch_model->lappy_sub_search();
                $data['lappysubcnt'] = $this->hotdealsearch_model->laptopcomputer_sub_search();
                $data['login_status'] =$login_status;
                    $data['login'] = $login;
                    $data['favourite_list']=$favourite_list;

                /*business and consumer count for pets*/
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_lappy();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_lappy();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_lappy();
        ...
        public function search_filters(){
             if($this->input->post()){
                $this->session->unset_userdata('lappy_sub');
                ...
                if($this->input->post('lappy_sub')){
                   // $data['lappy_sub'] = $this->input->post('lappy_sub');
                       $this->session->set_userdata('lappy_sub',$this->input->post('lappy_sub'));
                }else{
                     $this->session->set_userdata('lappy_sub',array());
                }
            ...
            $config = array();
            $config['base_url'] = base_url().'laptop_computers_view/search_filters';
            $config['total_rows'] = count($this->hotdealsearch_model->count_lappy_search());
            $config['per_page'] = 30;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->lappy_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*motor sub*/
                $result['lappy_sub'] = $this->hotdealsearch_model->lappy_sub_search();
                $result['lappysubcnt'] = $this->hotdealsearch_model->laptopcomputer_sub_search();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_lappy();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_lappy();
                 /*packages count*/
                $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_lappy();
