 ...
 */
class  J_front_office_help_deskview extends CI_Controller{
        ...
        public function index(){
             ...
             $config = array();
            $config['base_url'] = base_url().'j_front_office_help_deskview/index';
            $config['total_rows'] = count($this->postad_job_model->count_jfront_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
             ...
             if ($this->session->userdata('login_id') == '') {
                ...
                $jobs_view = $this->postad_job_model->jfront_view($search_option);
            ...
            $public_adview = $this->classifed_model->publicads_jobs();
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "j_front_office_help_deskview",
                        'log_name' => $log_name,
                ...
                $data['busconcount'] = $this->postad_job_model->busconcount_jfront();
                 /*seller and needed count for jobs*/
                $data['sellerneededcount'] = $this->postad_job_model->sellerneeded_jfront();
                /*packages count jobs*/
                $data['deals_pck'] = $this->postad_job_model->deals_pck_jfront();
                 /*packages count jobs*/
                $data['jobpositioncnt'] = $this->postad_job_model->position_jfront();
        ...
        public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'j_front_office_help_deskview/search_filters';
            $config['total_rows'] = count($this->postad_job_model->count_jfront_search());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
            $res = $this->postad_job_model->jfront_search($search_option);
            ...
            $result['paging_links'] = $this->pagination->create_links();
             $result['jobs_sub'] = $this->hotdealsearch_model->jobs_sub_search();
                 /*business and consumer count for jobs*/
            $result['busconcount'] = $this->postad_job_model->busconcount_jfront();
             /*seller and needed count for jobs*/
            $result['sellerneededcount'] = $this->postad_job_model->sellerneeded_jfront();
            /*packages count jobs*/
            $result['deals_pck'] = $this->postad_job_model->deals_pck_jfront();
             /*packages count jobs*/
            $result['jobpositioncnt'] = $this->postad_job_model->position_jfront();
