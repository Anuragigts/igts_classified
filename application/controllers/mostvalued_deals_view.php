...
class Mostvalued_deals_view extends CI_Controller{
        ...
        public function index(){
                ...
                $config = array();
                $config['base_url'] = base_url().'mostvalued_deals_view/index';
                $config['total_rows'] = count($this->classifed_model->count_viewallmostads());
                $config['per_page'] = 30;
                ...
                $viewall_mostads = $this->classifed_model->viewall_mostads($search_option);
                foreach ($viewall_mostads as $sview) {
                ...
                $data   =   array(
                        ...
                        'log_name' => $log_name,
                        'login' =>$login,
                        "viewall_mostads" => $viewall_mostads,
        ...
        public function search_filters(){
             ...
             $config = array();
            $config['base_url'] = base_url().'mostvalued_deals_view/search_filters';
            $config['total_rows'] = count($this->classifed_model->count_viewall_mostads1());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            $viewall_mostads = $this->classifed_model->viewall_mostads1($search_option);
                foreach ($viewall_mostads as $sview) {
                    $loginid = $sview->login_id;
                }
                 if ($this->session->userdata('login_id') == '') {
