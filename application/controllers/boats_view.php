 ...
 */
class  Boats_view extends CI_Controller{
        ...
        public function index(){
            ...
            $search_option = array(
                ...
                $boats_result = $this->classifed_model->boats_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "boats_view",
                         "boats_result" => $boats_result,
                         'log_name' => $log_name,
                         'paging_links' =>$this->pagination->create_links()
                );
                
                /*motor*/
                $data['boats_sub'] = $this->hotdealsearch_model->boats_sub_search();
                $data['login_status'] =$login_status;
                    $data['login'] = $login;
                    $data['favourite_list']=$favourite_list;

                /*business and consumer count for pets*/
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_boats();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_boats();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_boats();
        ...
        public function search_filters(){
             if($this->input->post()){
                ...
                if($this->input->post('boats_sub')){
                   // $data['boats_sub'] = $this->input->post('boats_sub');
                       $this->session->set_userdata('boats_sub',$this->input->post('boats_sub'));
                }else{
                     $this->session->set_userdata('boats_sub',array());
                }
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->boats_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*motor sub*/
                 $result['boats_sub'] = $this->hotdealsearch_model->boats_sub_search();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_boats();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_boats();
                 /*packages count*/
                $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_boats();
