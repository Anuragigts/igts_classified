 ...
 */
class  Network_components_view  extends CI_Controller{
        ...
        public function index(){
            $this->session->set_userdata('network_sub',array());
            ...
            $config = array();
            $config['base_url'] = base_url().'network_components_view /index';
            $config['total_rows'] = count($this->hotdealsearch_model->count_networkcomponents_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $poto_result = $this->hotdealsearch_model->networkcomponents_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "network_components_view",
                ...
                /*motor*/
                $data['networkssub'] = $this->hotdealsearch_model->networks_sub_search();
                ...
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_networkcomponents();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_networkcomponents();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_networkcomponents();
        ...
        public function search_filters(){
             if($this->input->post()){
                $this->session->unset_userdata('network_sub');
                ...
                if($this->input->post('network_sub')){
                       $this->session->set_userdata('network_sub',$this->input->post('network_sub'));
                }else{
                     $this->session->set_userdata('network_sub',array());
                }
            ...
            $config = array();
            $config['base_url'] = base_url().'network_components_view/search_filters';
            $config['total_rows'] = count($this->hotdealsearch_model->count_networkcomponents_search());
            $config['per_page'] = 30;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->networkcomponents_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*motor sub*/
                $result['networkssub'] = $this->hotdealsearch_model->networks_sub_search();
                $result['potosubcnt'] = $this->hotdealsearch_model->photography_search();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_networkcomponents();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_networkcomponents();
                 /*packages count*/
                $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_networkcomponents();
