 ...
 */
class  Bikes_scoters_view extends CI_Controller{
        ...
        public function index(){
            ...
            $config = array();
            $config['base_url'] = base_url().'bikes_scoters_view/index';
            $config['total_rows'] = count($this->classifed_model->count_bikes_scoters_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $bikes_result = $this->classifed_model->bikes_scoters_view($search_option);
            foreach ($bikes_result as $pview) {
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "bikes_scoters_view",
                         "bikes_result" => $bikes_result,
                         'log_name' => $log_name,
                         'paging_links' =>$this->pagination->create_links()
                );
                
                /*motor*/
                 $data['motor_sub'] = $this->hotdealsearch_model->motorbikes_cnt();
                $data['petrolcnt'] = $this->hotdealsearch_model->bikepetrolcnt();
                $data['milagecnt'] = $this->hotdealsearch_model->bikemilagecnt();
                $data['enginecnt'] = $this->hotdealsearch_model->bikeenginecnt();
                $data['login_status'] =$login_status;
                    $data['login'] = $login;
                    $data['favourite_list']=$favourite_list;

                /*business and consumer count for pets*/
                 $data['busconcount'] = $this->hotdealsearch_model->busconcount_bikes();
                 /*seller and needed count for pets*/
               $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_bikes();
                 /*packages count*/
               $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_bikes();
        ...
        public function search_filters(){
             if($this->input->post()){
                ...
                if($this->input->post('bikes_sub')){
                       $this->session->set_userdata('bikes_sub',$this->input->post('bikes_sub'));
                }else{
                     $this->session->set_userdata('bikes_sub',array());
                }
            ...
            $config = array();
            $config['base_url'] = base_url().'bikes_scoters_view/search_filters';
            $config['total_rows'] = count($this->hotdealsearch_model->count_bikes_search());
            $config['per_page'] = 30;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->bikes_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
              ...
              $result   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "bikes_scoters_view");
            $result['bikes_result'] = $rs;
            $public_adview = $this->classifed_model->publicads_motor();
            $result['petrolcnt'] = $this->hotdealsearch_model->bikepetrolcnt();
            $result['milagecnt'] = $this->hotdealsearch_model->bikemilagecnt();
            $result['enginecnt'] = $this->hotdealsearch_model->bikeenginecnt();
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid' "), 0, 'first_name');
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*motor sub*/
                $result['motor_sub'] = $this->hotdealsearch_model->motorbikes_cnt();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_bikes();
                 /*seller and needed count for pets*/
               $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_bikes();
                 /*packages count*/
               $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_bikes();
