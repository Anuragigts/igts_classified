...
class  Baby_girl_view extends CI_Controller{
        ...
        $baby_girl_view = $this->classifed_model->baby_girl_view($search_option);
        ...
        //$data["paging_links"] =  $this->pagination->create_links();
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid' "), 0, 'first_name');
                $data   =   array(
                        ...
                        "babygirlview_result" => $baby_girl_view,
                ...
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_babygirlview();
                /*service provided / needed for services*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_babygirlview();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_babygirlview();
        ...
        // $services_view = $this->classifed_model->services_view($search_option);
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->babygirl_view_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
                        $loginid = $sview->login_id;
                    }
