 ...
 */
class  Farmingvehicles_view extends CI_Controller{
        ...
        public function index(){
            $this->session->set_userdata('farming_sub',array());
            ...
            $config = array();
            $config['base_url'] = base_url().'farmingvehicles_view/index';
            $config['total_rows'] = count($this->classifed_model->count_farming_view());
            $config['per_page'] = 2;
            ...
            $search_option = array(
                ...
                $farming_result = $this->classifed_model->farming_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                ...
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_farming();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_farming();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_farming();
        ...
        public function search_filters(){
             if($this->input->post()){
                $this->session->unset_userdata('farming_sub');
                ...
                if($this->input->post('farming_sub')){
                   // $data['farming_sub'] = $this->input->post('farming_sub');
                       $this->session->set_userdata('farming_sub',$this->input->post('farming_sub'));
                }else{
                     $this->session->set_userdata('farming_sub',array());
                }
            ...
            $config = array();
            $config['base_url'] = base_url().'farmingvehicles_view/search_filters';
            $config['total_rows'] = count($this->hotdealsearch_model->count_farming_search());
            $config['per_page'] = 2;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->farming_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*motor sub*/
                $result['farming_sub'] = $this->hotdealsearch_model->farming_sub_search();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_farming();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_farming();
                 /*packages count*/
                $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_farming();
