 ...
 */
class  Plantmachinery_view extends CI_Controller{
        ...
        public function index(){
            $this->session->set_userdata('plants_sub',array());
            ...
            $config = array();
            $config['base_url'] = base_url().'plantmachinery_view/index';
            $config['total_rows'] = count($this->classifed_model->count_plants_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $plants_result = $this->classifed_model->plants_view($search_option);
            foreach ($plants_result as $pview) {
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                ...
                /*motor*/
                $data['plants_sub'] = $this->hotdealsearch_model->plants_sub_search();
                $data['login_status'] =$login_status;
                    $data['login'] = $login;
                    $data['favourite_list']=$favourite_list;

                /*business and consumer count for pets*/
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_plants();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_plants();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_plants();
        ...
        public function search_filters(){
             if($this->input->post()){
                $this->session->unset_userdata('plants_sub');
                ...
                $this->session->unset_userdata('longg');
                 if($this->input->post('plants_sub')){
                   // $data['plants_sub'] = $this->input->post('plants_sub');
                       $this->session->set_userdata('plants_sub',$this->input->post('plants_sub'));
                }else{
                     $this->session->set_userdata('plants_sub',array());
                }
            ...
            $config = array();
            $config['base_url'] = base_url().'plantmachinery_view/search_filters';
            $config['total_rows'] = count($this->hotdealsearch_model->count_plants_search());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->plants_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*motor sub*/
                $result['plants_sub'] = $this->hotdealsearch_model->plants_sub_search();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_plants();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_plants();
                 /*packages count*/
                $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_plants();
