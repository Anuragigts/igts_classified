 ...
 */
class  S_coachings_trainingview extends CI_Controller{
        ...
        public function index(){
               $this->session->set_userdata('prof_service',array());
                ...
                $config = array();
                $config['base_url'] = base_url().'s_coachings_trainingview/index';
                $config['total_rows'] = count($this->postad_model->count_profcoach_view());
                $config['per_page'] = 30;
        ...
        $services_view = $this->postad_model->profcoach_view($search_option);
        ...
        //$data["paging_links"] =  $this->pagination->create_links();
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "s_coachings_trainingview",
                ...
                $data['busconcount'] = $this->postad_model->busconcount_profcoach();
                /*service provided / needed for services*/
                $data['sellerneededcount'] = $this->postad_model->sellerneeded_profcoach();
                 /*packages count*/
                $data['deals_pck'] = $this->postad_model->deals_pck_profcoach();
        ...
        public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'s_coachings_trainingview/search_filters';
            $config['total_rows'] = count($this->postad_model->count_profcoach_search());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
        ...
        $services_view = $this->postad_model->profcoach_search($search_option);
