 ...
 */
class  Men_accessoriesview extends CI_Controller{
        ...
        public function index(){
                ...
                $config = array();
                $config['base_url'] = base_url().'men_accessoriesview/index';
                $config['total_rows'] = count($this->postad_cloths_model->count_maccessories_view());
                $config['per_page'] = 30;
        ...
        $clothstyle_view = $this->postad_cloths_model->maccessories_view($search_option);
        ...
        //$data["paging_links"] =  $this->pagination->create_links();
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid' "), 0, 'first_name');
                ...
                $data['busconcount'] = $this->postad_cloths_model->busconcount_maccessories();
                /*service provided / needed for services*/
                $data['sellerneededcount'] = $this->postad_cloths_model->sellerneeded_maccessories();
                 /*packages count*/
                $data['deals_pck'] = $this->postad_cloths_model->deals_pck_maccessories();
        ...
        public function search_filters(){
            if($this->input->post()){
             ...
             $services_view = $this->postad_cloths_model->count_maccessories_search();
             // echo count($services_view);
            $config = array();
            $config['base_url'] = base_url().'men_accessoriesview/search_filters';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->postad_cloths_model->maccessories_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
                        $loginid = $sview->login_id;
                    }
