 ...
 */
class  Vans_trunks_svu_view extends CI_Controller{
        ...
        public function index(){
            $this->session->set_userdata('vansuv_sub',array());
            ...
            $config = array();
            $config['base_url'] = base_url().'vans_trunks_svu_view/index';
            $config['total_rows'] = count($this->classifed_model->count_vans_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $vans_result = $this->classifed_model->vans_view($search_option);
            foreach ($vans_result as $pview) {
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        ...
                        "content"   =>  "vans_trunks_svu_view",
                         "vans_result" => $vans_result,
                         'log_name' => $log_name,
                         'paging_links' =>$this->pagination->create_links()
                );
                
                /*motor*/
                $data['motor_sub'] = $this->hotdealsearch_model->vansuv_cnt();
                $data['petrolcnt'] = $this->hotdealsearch_model->vans_petrolcnt();
                $data['milagecnt'] = $this->hotdealsearch_model->vans_milagecnt();
                $data['enginecnt'] = $this->hotdealsearch_model->vans_enginecnt();
                $data['login_status'] =$login_status;
                $data['login'] = $login;
                $data['favourite_list']=$favourite_list;

                /*business and consumer count for pets*/
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_vans();
                 /*seller and needed count for pets*/
               $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_vans();
                 /*packages count*/
               $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_vans();
        ...
        public function search_filters(){
             if($this->input->post()){
                $this->session->unset_userdata('vansuv_sub');
                ...
                if($this->input->post('vansuv_sub')){
                       $this->session->set_userdata('vansuv_sub',$this->input->post('vansuv_sub'));
                }else{
                     $this->session->set_userdata('vansuv_sub',array());
                }
            ...
            $config = array();
            $config['base_url'] = base_url().'vans_trunks_svu_view/search_filters';
            $config['total_rows'] = count($this->hotdealsearch_model->count_vans_search());
            $config['per_page'] = 30;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->vans_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*motor sub*/
                $result['motor_sub'] = $this->hotdealsearch_model->vansuv_cnt();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_vans();
                 /*seller and needed count for pets*/
               $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_vans();
                 /*packages count*/
               $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_vans();
