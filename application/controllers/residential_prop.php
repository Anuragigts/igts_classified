 ...
 */
class  Residential_prop extends CI_Controller{
        ...
        public function index(){
            ...
            $config = array();
            $config['base_url'] = base_url().'residential_prop/index';
            $config['total_rows'] = count($this->classifed_model->count_find_propertyresi_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $residential_view = $this->classifed_model->find_propertyresi_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid' "), 0, 'first_name');
               ...
               $data['login_status'] =$login_status;
                    ...
                    /*area build count*/
                    $data['areacount'] = $this->hotdealsearch_model->areacount_propertyresi();
                    /*bedrooms count*/
                    $data['bedroomcount'] = $this->hotdealsearch_model->bedroomcount_propertyresi();
                    /*bathrooms count*/
                    $data['bathroomcount'] = $this->hotdealsearch_model->bathroomcount_propertyresi();
                ...
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_propertyresi();
                 /*seller and needed count for findproperty*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_property1resi();
                 /*packages count*/
               ...
               $data['subcat_resi'] = $this->hotdealsearch_model->subcat_resi_deals();
         ...
         public function search_filters(){
            if($this->input->post()){
             ...
             $config = array();
            $config['base_url'] = base_url().'residential_prop/search_filters';
            $config['total_rows'] = count($this->hotdealsearch_model->count_propertyresi_search());
            $config['per_page'] = 30;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
           ...
           $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->propertyresi_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            /*area build count*/
                $result['areacount'] = $this->hotdealsearch_model->areacount_propertyresi();
                /*bedrooms count*/
                $result['bedroomcount'] = $this->hotdealsearch_model->bedroomcount_propertyresi();
                /*bathrooms count*/
                $result['bathroomcount'] = $this->hotdealsearch_model->bathroomcount_propertyresi();
                /*residential and commercial count*/
