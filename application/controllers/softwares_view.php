 ...
 */
class  Softwares_view extends CI_Controller{
        ...
        public function index(){
            ...
            $config = array();
            $config['base_url'] = base_url().'softwares_view/index';
            $config['total_rows'] = count($this->hotdealsearch_model->count_softwares_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $poto_result = $this->hotdealsearch_model->softwares_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                ...
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_softwares();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_softwares();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_softwares();
        ...
        public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'softwares_view/search_filters';
            $config['total_rows'] = count($this->hotdealsearch_model->count_softwares_search());
            $config['per_page'] = 30;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->softwares_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              ...
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_softwares();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_softwares();
                 /*packages count*/
                $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_softwares();
