 ...
 */
class  Home_entertainment_view extends CI_Controller{
        ...
        public function index(){
            $this->session->set_userdata('entertain_sub',array());
            ...
            $config = array();
            $config['base_url'] = base_url().'home_entertainment_view/index';
            $config['total_rows'] = count($this->classifed_model->count_entertain_view());
            $config['per_page'] = 2;
            ...
            $search_option = array(
                ...
                $smalls_result = $this->classifed_model->entertain_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "home_entertainment_view",
                         "entertain_result" => $smalls_result,
                         'log_name' => $log_name,
                ...
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_entertain();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_entertain();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_entertain();
        ...
        public function search_filters(){
             if($this->input->post()){
                $this->session->unset_userdata('entertain_sub');
                ...
                if($this->input->post('entertain_sub')){
                   // $data['entertain_sub'] = $this->input->post('entertain_sub');
                       $this->session->set_userdata('entertain_sub',$this->input->post('entertain_sub'));
                }else{
                     $this->session->set_userdata('entertain_sub',array());
                }
            ...
            $config = array();
            $config['base_url'] = base_url().'home_entertainment_view/search_filters';
            ...
            $result['paging_links'] = $this->pagination->create_links();
              ...
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_entertain();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_entertain();
                 /*packages count*/
                $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_entertain();
