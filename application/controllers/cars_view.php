...
class  Cars_view extends CI_Controller{
        ...
        public function index(){
            $this->session->set_userdata('cars_sub',array());
            ...
            $config = array();
            $config['base_url'] = base_url().'cars_view/index';
            $config['total_rows'] = count($this->classifed_model->count_cars_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $cars_result = $this->classifed_model->cars_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                ...
                /*motor*/
                $data['motor_sub'] = $this->hotdealsearch_model->motorcars_cnt();
                $data['petrolcnt'] = $this->hotdealsearch_model->petrolcnt();
                $data['milagecnt'] = $this->hotdealsearch_model->milagecnt();
                $data['enginecnt'] = $this->hotdealsearch_model->enginecnt();
                $data['login_status'] =$login_status;
                    $data['login'] = $login;
                    $data['favourite_list']=$favourite_list;

                /*business and consumer count for pets*/
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_cars();
                 /*seller and needed count for pets*/
               $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_cars();
                 /*packages count*/
               $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_cars();
        ...
        public function search_filters(){
             if($this->input->post()){
                $this->session->unset_userdata('cars_sub');
                ...
                if($this->input->post('cars_sub')){
                       $this->session->set_userdata('cars_sub',$this->input->post('cars_sub'));
                }else{
                     $this->session->set_userdata('cars_sub',array());
                }
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->cars_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $public_adview = $this->classifed_model->publicads_motor();
            $result['motor_sub'] = $this->hotdealsearch_model->motorcars_cnt();
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_cars();
                 /*seller and needed count for pets*/
               $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_cars();
                 /*packages count*/
               $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_cars();
