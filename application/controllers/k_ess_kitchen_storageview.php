 ...
 */
class  K_ess_kitchen_storageview extends CI_Controller{
        ...
        public function index(){
            ...
            $config = array();
            $config['base_url'] = base_url().'k_ess_kitchen_storageview/index';
            $config['total_rows'] = count($this->postad_kitchen_model->count_kstorage_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $kitchenhome_view = $this->postad_kitchen_model->kstorage_view($search_option);
                ...
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "k_ess_kitchen_storageview",
                ...
                $data['busconcount'] = $this->postad_kitchen_model->busconcount_kstorage();
                 /*seller and needed count for kitchen*/
                $data['sellerneededcount'] = $this->postad_kitchen_model->sellerneeded_kstorage();
                 /*packages count*/
                $data['deals_pck'] = $this->postad_kitchen_model->deals_pck_kstorage();
         ...
         public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'k_ess_kitchen_storageview/search_filters';
            $config['total_rows'] = count($this->postad_kitchen_model->count_kstorage_search());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             ...
             $rs = $this->postad_kitchen_model->kstorage_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['favourite_list']=$favourite_list;
             /*business and consumer count for kitchen*/
                $result['busconcount'] = $this->postad_kitchen_model->busconcount_kstorage();
                 /*seller and needed count for kitchen*/
                $result['sellerneededcount'] = $this->postad_kitchen_model->sellerneeded_kstorage();
                 /*packages count*/
                $result['deals_pck'] = $this->postad_kitchen_model->deals_pck_kstorage();
