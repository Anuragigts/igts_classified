...
class  H_ess_dining_room_furview extends CI_Controller{
        ...
        public function index(){
            ...
            $config['total_rows'] = count($this->postad_kitchen_model->count_hdining_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $kitchenhome_view = $this->postad_kitchen_model->hdining_view($search_option);
                ...
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "h_ess_dining_room_furview",
                ...
                $data['busconcount'] = $this->postad_kitchen_model->busconcount_hdining();
                 /*seller and needed count for kitchen*/
                $data['sellerneededcount'] = $this->postad_kitchen_model->sellerneeded_hdiving();
                 /*packages count*/
                $data['deals_pck'] = $this->postad_kitchen_model->deals_pck_hdining();
         ...
         public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'h_ess_dining_room_furview/search_filters';
            $config['total_rows'] = count($this->postad_kitchen_model->count_hdining_search());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             ...
             $rs = $this->postad_kitchen_model->hdining_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['favourite_list']=$favourite_list;
             /*business and consumer count for kitchen*/
                $result['busconcount'] = $this->postad_kitchen_model->busconcount_hdining();
                 /*seller and needed count for kitchen*/
                $result['sellerneededcount'] = $this->postad_kitchen_model->sellerneeded_hdiving();
                 /*packages count*/
                $result['deals_pck'] = $this->postad_kitchen_model->deals_pck_hdining();
