 ...
 */
class  H_ess_carpets_flooringview extends CI_Controller{
        ...
        public function index(){
            ...
            $config = array();
            $config['base_url'] = base_url().'h_ess_carpets_flooringview/index';
            $config['total_rows'] = count($this->postad_kitchen_model->count_hcarpets_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $kitchenhome_view = $this->postad_kitchen_model->hcarpets_view($search_option);
                ...
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "h_ess_carpets_flooringview",
                ...
                $data['busconcount'] = $this->postad_kitchen_model->busconcount_hcarpets();
                 /*seller and needed count for kitchen*/
                $data['sellerneededcount'] = $this->postad_kitchen_model->sellerneeded_hcarpet();
                 /*packages count*/
                $data['deals_pck'] = $this->postad_kitchen_model->deals_pck_hcarpet();
         ...
         public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'h_ess_carpets_flooringview/search_filters';
            $config['total_rows'] = count($this->postad_kitchen_model->count_hcarpets_search());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             ...
             $rs = $this->postad_kitchen_model->hcarpets_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['favourite_list']=$favourite_list;
             /*business and consumer count for kitchen*/
                $result['busconcount'] = $this->postad_kitchen_model->busconcount_hcarpets();
                 /*seller and needed count for kitchen*/
                $result['sellerneededcount'] = $this->postad_kitchen_model->sellerneeded_hcarpet();
                 /*packages count*/
                $result['deals_pck'] = $this->postad_kitchen_model->deals_pck_hcarpet();
