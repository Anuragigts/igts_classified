 ...
 */
class  S_business_servicesview extends CI_Controller{
        ...
        public function index(){
               $this->session->set_userdata('prof_service',array());
                ...
                $config = array();
                $config['base_url'] = base_url().'s_business_servicesview/index';
                $config['total_rows'] = count($this->postad_model->count_profbusiness_view());
                $config['per_page'] = 30;
        ...
        $services_view = $this->postad_model->profbusiness_view($search_option);
        ...
        //$data["paging_links"] =  $this->pagination->create_links();
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "s_business_servicesview",
                ...
                $data['busconcount'] = $this->postad_model->busconcount_profbusiness();
                /*service provided / needed for services*/
                $data['sellerneededcount'] = $this->postad_model->sellerneeded_profbusiness();
                 /*packages count*/
                $data['deals_pck'] = $this->postad_model->deals_pck_profbusiness();
        ...
        public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'s_business_servicesview/search_filters';
            $config['total_rows'] = count($this->postad_model->count_profbusiness_search());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
        ...
        $services_view = $this->postad_model->profbusiness_search($search_option);
