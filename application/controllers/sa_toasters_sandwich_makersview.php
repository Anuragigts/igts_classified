 ...
 */
class  Sa_toasters_sandwich_makersview extends CI_Controller{
       public function __construct(){
        ...
        public function index(){
            ...
            $config = array();
            $config['base_url'] = base_url().'sa_toasters_sandwich_makersview/index';
            $config['total_rows'] = count($this->postad_ezone_model->count_toastersandwichmakers_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $ezone_result = $this->postad_ezone_model->toastersandwichmakers_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "sa_toasters_sandwich_makersview",
                ...
                $data['busconcount'] = $this->postad_ezone_model->busconcount_toastersandwichmakers();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->postad_ezone_model->sellerneeded_toastersandwichmakers();
                 /*packages count*/
                $data['deals_pck'] = $this->postad_ezone_model->deals_pck_toastersandwichmakers();
        ...
        public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'sa_toasters_sandwich_makersview/search_filters';
            $config['total_rows'] = count($this->postad_ezone_model->count_toastersandwichmakers_search());
            $config['per_page'] = 30;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->postad_ezone_model->toastersandwichmakers_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              ...
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->postad_ezone_model->busconcount_toastersandwichmakers();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->postad_ezone_model->sellerneeded_toastersandwichmakers();
                 /*packages count*/
                $result['deals_pck'] = $this->postad_ezone_model->deals_pck_toastersandwichmakers();
