 ...
 */
class  Personal_care_view extends CI_Controller{
        ...
        public function index(){
            $this->session->set_userdata('pcare_sub',array());
            ...
            $search_option = array(
                ...
                $smalls_result = $this->classifed_model->pcare_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                ...
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_pcare();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_pcare();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_pcare();
        ...
        public function search_filters(){
             if($this->input->post()){
                $this->session->unset_userdata('pcare_sub');
                ...
                if($this->input->post('pcare_sub')){
                   // $data['pcare_sub'] = $this->input->post('pcare_sub');
                       $this->session->set_userdata('pcare_sub',$this->input->post('pcare_sub'));
                }else{
                     $this->session->set_userdata('pcare_sub',array());
                }
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->pcare_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              ...
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_pcare();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_pcare();
                 /*packages count*/
                $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_pcare();
