 ...
 */
class Recent_deals_view extends CI_Controller{
       public function __construct(){
        ...
        public function index(){
                ...
                $config = array();
                $config['base_url'] = base_url().'recent_deals_view/index';
                $config['total_rows'] = count($this->classifed_model->count_viewallconsumer());
                $config['per_page'] = 30;
                ...
                $viewall_consumer = $this->classifed_model->viewall_consumer($search_option);
                foreach ($viewall_consumer as $sview) {
                ...
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "recent_deals_view",
                        'log_name' => $log_name,
                        'login' =>$login,
                        "viewall_consumer" => $viewall_consumer,
        ...
        public function search_filters(){
             ...
             $config = array();
            $config['base_url'] = base_url().'recent_deals_view/search_filters';
            $config['total_rows'] = count($this->classifed_model->count_viewall_consumer1());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            $viewall_consumer = $this->classifed_model->viewall_consumer1($search_option);
                foreach ($viewall_consumer as $sview) {
                    $loginid = $sview->login_id;
                }
                 if ($this->session->userdata('login_id') == '') {
