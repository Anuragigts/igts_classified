 ...
 */
class  A_mobile_tablet_accessoriesview extends CI_Controller{
       public function __construct(){
        ...
        public function index(){
          $this->session->set_userdata('access_sub',array());
            ...
            $config = array();
            $config['base_url'] = base_url().'a_mobile_tablet_accessoriesview/index';
            $config['total_rows'] = count($this->postad_ezone_model->count_amobiletablet_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $ezone_result = $this->postad_ezone_model->amobiletablet_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "a_mobile_tablet_accessoriesview",
                ...
                /*motor*/
                $data['ezone_sub'] = $this->hotdealsearch_model->accesories_mtablets();
                $data['login_status'] =$login_status;
                    $data['login'] = $login;
                    $data['favourite_list']=$favourite_list;

                /*business and consumer count for pets*/
                $data['busconcount'] = $this->postad_ezone_model->busconcount_amobiletablet();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->postad_ezone_model->sellerneeded_amobiletablet();
                 /*packages count*/
                $data['deals_pck'] = $this->postad_ezone_model->deals_pck_amobiletablet();
        ...
        public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'a_mobile_tablet_accessoriesview/search_filters';
            $config['total_rows'] = count($this->postad_ezone_model->count_amobiletablet_search());
            $config['per_page'] = 30;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->postad_ezone_model->amobiletablet_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*motor sub*/
                $result['ezone_sub'] = $this->hotdealsearch_model->accesories_mtablets();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->postad_ezone_model->busconcount_amobiletablet();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->postad_ezone_model->sellerneeded_amobiletablet();
                 /*packages count*/
                $result['deals_pck'] = $this->postad_ezone_model->deals_pck_amobiletablet();
