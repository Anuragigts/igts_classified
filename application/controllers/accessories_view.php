 ...
 */
class  Accessories_view extends CI_Controller{
        ...
        public function index(){
            ...
            $search_option = array(
                ...
                $smalls_result = $this->classifed_model->access_view($search_option);
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                ...
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_access();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_access();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_access();
        ...
        public function search_filters(){
             if($this->input->post()){
                ...
                if($this->input->post('access_sub')){
                   // $data['access_sub'] = $this->input->post('access_sub');
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->access_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              ...
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_access();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_access();
                 /*packages count*/
                $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_access();
