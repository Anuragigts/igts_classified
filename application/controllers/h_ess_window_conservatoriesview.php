 ...
 */
class  H_ess_window_conservatoriesview extends CI_Controller{
        ...
        public function index(){
            ...
            $config = array();
            $config['base_url'] = base_url().'h_ess_window_conservatoriesview/index';
            $config['total_rows'] = count($this->postad_kitchen_model->count_hwind_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $kitchenhome_view = $this->postad_kitchen_model->hwind_view($search_option);
                ...
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "h_ess_window_conservatoriesview",
                ...
                $data['busconcount'] = $this->postad_kitchen_model->busconcount_hwind();
                 /*seller and needed count for kitchen*/
                $data['sellerneededcount'] = $this->postad_kitchen_model->sellerneeded_hwind();
                 /*packages count*/
                $data['deals_pck'] = $this->postad_kitchen_model->deals_pck_hwind();
         ...
         public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'h_ess_window_conservatoriesview/search_filters';
            $config['total_rows'] = count($this->postad_kitchen_model->count_hwind_search());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             ...
             $rs = $this->postad_kitchen_model->hwind_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['favourite_list']=$favourite_list;
             /*business and consumer count for kitchen*/
                $result['busconcount'] = $this->postad_kitchen_model->busconcount_hwind();
                 /*seller and needed count for kitchen*/
                $result['sellerneededcount'] = $this->postad_kitchen_model->sellerneeded_hwind();
                 /*packages count*/
                $result['deals_pck'] = $this->postad_kitchen_model->deals_pck_hwind();
