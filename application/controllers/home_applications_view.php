 ...
 */
class  Home_applications_view extends CI_Controller{
        ...
        public function index(){
            $this->session->set_userdata('homes_sub',array());
            ...
            $config = array();
            $config['base_url'] = base_url().'home_applications_view/index';
            $config['total_rows'] = count($this->classifed_model->count_homes_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $homes_result = $this->classifed_model->homes_view($search_option);
            foreach ($homes_result as $pview) {
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "home_applications_view",
                ...
                /*motor*/
                $data['homes_sub'] = $this->hotdealsearch_model->homes_sub_search();
                $data['happ_sub'] = $this->hotdealsearch_model->happliance_sub_search();
                $data['login_status'] =$login_status;
                    $data['login'] = $login;
                    $data['favourite_list']=$favourite_list;

                /*business and consumer count for pets*/
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_homes();
                 /*seller and needed count for pets*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_homes();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_homes();
        ...
        public function search_filters(){
             if($this->input->post()){
                $this->session->unset_userdata('homes_sub');
                ...
                if($this->input->post('homes_sub')){
                   // $data['homes_sub'] = $this->input->post('homes_sub');
                       $this->session->set_userdata('homes_sub',$this->input->post('homes_sub'));
                }else{
                     $this->session->set_userdata('homes_sub',array());
                }
            ...
            $config = array();
            $config['base_url'] = base_url().'home_applications_view/search_filters';
            $config['total_rows'] = count($this->hotdealsearch_model->count_homes_search());
            $config['per_page'] = 30;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->homes_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*motor sub*/
                $result['homes_sub'] = $this->hotdealsearch_model->homes_sub_search();
                $result['happ_sub'] = $this->hotdealsearch_model->happliance_sub_search();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_homes();
                 /*seller and needed count for pets*/
                $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_homes();
                 /*packages count*/
                $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_homes();
