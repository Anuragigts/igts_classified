 ...
 */
class  J_electronics_electrical_ngineeringview extends CI_Controller{
        ...
        public function index(){
             ...
             $config = array();
            $config['base_url'] = base_url().'j_electronics_electrical_ngineeringview/index';
            $config['total_rows'] = count($this->postad_job_model->count_jelectro_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
             ...
             if ($this->session->userdata('login_id') == '') {
                ...
                $jobs_view = $this->postad_job_model->jelectro_view($search_option);
            ...
            $public_adview = $this->classifed_model->publicads_jobs();
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "j_electronics_electrical_ngineeringview",
                        'log_name' => $log_name,
                ...
                $data['busconcount'] = $this->postad_job_model->busconcount_jelectro();
                 /*seller and needed count for jobs*/
                $data['sellerneededcount'] = $this->postad_job_model->sellerneeded_jelectro();
                /*packages count jobs*/
                $data['deals_pck'] = $this->postad_job_model->deals_pck_jelectro();
                 /*packages count jobs*/
                $data['jobpositioncnt'] = $this->postad_job_model->position_jelectro();
        ...
        public function search_filters(){
            ...
            $config = array();
            $config['base_url'] = base_url().'j_electronics_electrical_ngineeringview/search_filters';
            $config['total_rows'] = count($this->postad_job_model->count_jelectro_search());
            $config['per_page'] = 30;
            $config['next_link'] = 'Next';
            $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
            $res = $this->postad_job_model->jelectro_search($search_option);
            ...
            $result['paging_links'] = $this->pagination->create_links();
             $result['jobs_sub'] = $this->hotdealsearch_model->jobs_sub_search();
                 /*business and consumer count for jobs*/
            $result['busconcount'] = $this->postad_job_model->busconcount_jelectro();
             /*seller and needed count for jobs*/
            $result['sellerneededcount'] = $this->postad_job_model->sellerneeded_jelectro();
            /*packages count jobs*/
            $result['deals_pck'] = $this->postad_job_model->deals_pck_jelectro();
             /*packages count jobs*/
            $result['jobpositioncnt'] = $this->postad_job_model->position_jelectro();
