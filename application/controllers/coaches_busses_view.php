 ...
 */
class  Coaches_busses_view extends CI_Controller{
        ...
        public function index(){
            $this->session->set_userdata('coachbus_sub',array());
            ...
            $config = array();
            $config['base_url'] = base_url().'coaches_busses_view/index';
            $config['total_rows'] = count($this->classifed_model->count_coaches_view());
            $config['per_page'] = 30;
            ...
            $search_option = array(
                ...
                $coaches_result = $this->classifed_model->coaches_view($search_option);
            foreach ($coaches_result as $pview) {
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid'"), 0, 'first_name');
                $data   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "coaches_busses_view",
                         "coaches_result" => $coaches_result,
                         'log_name' => $log_name,
                         'paging_links' =>$this->pagination->create_links()
                );
                
                /*motor*/
                $data['motor_sub'] = $this->hotdealsearch_model->coachesbuscnt();
                $data['petrolcnt'] = $this->hotdealsearch_model->coaches_petrolcnt();
                $data['milagecnt'] = $this->hotdealsearch_model->coaches_milagecnt();
                $data['enginecnt'] = $this->hotdealsearch_model->coaches_enginecnt();
                $data['login_status'] =$login_status;
                $data['login'] = $login;
                $data['favourite_list']=$favourite_list;

                /*business and consumer count for pets*/
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_coaches();
                 /*seller and needed count for pets*/
               $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_coaches();
                 /*packages count*/
               $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_coaches();
        ...
        public function search_filters(){
             if($this->input->post()){
                $this->session->unset_userdata('coachbus_sub');
                ...
                if($this->input->post('coachbus_sub')){
                       $this->session->set_userdata('coachbus_sub',$this->input->post('coachbus_sub'));
                }else{
                     $this->session->set_userdata('coachbus_sub',array());
                }
            ...
            $config = array();
            $config['base_url'] = base_url().'coaches_busses_view/search_filters';
            $config['total_rows'] = count($this->hotdealsearch_model->count_coaches_search());
            $config['per_page'] = 30;
             $config['next_link'] = 'Next';
              $config['prev_link'] = 'Previous';
            $config['full_tag_open'] ='<div id="pagination" style="color:red;border:2px solid:blue">';
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->coaches_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
              ...
              $result   =   array(
                        "title"     =>  "Classifieds",
                        "content"   =>  "coaches_busses_view");
            $result['coaches_result'] = $rs;
            $public_adview = $this->classifed_model->publicads_motor();
            $result['petrolcnt'] = $this->hotdealsearch_model->coaches_petrolcnt();
            $result['milagecnt'] = $this->hotdealsearch_model->coaches_milagecnt();
            $result['enginecnt'] = $this->hotdealsearch_model->coaches_enginecnt();
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid' "), 0, 'first_name');
            ...
            $result['paging_links'] = $this->pagination->create_links();
              /*motor sub*/
                $result['motor_sub'] = $this->hotdealsearch_model->coachesbuscnt();
              /*business and consumer count for pets*/
                $result['busconcount'] = $this->hotdealsearch_model->busconcount_coaches();
                 /*seller and needed count for pets*/
               $result['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_coaches();
                 /*packages count*/
               $result['deals_pck'] = $this->hotdealsearch_model->deals_pck_coaches();
