 ...
 */
class  Boys_view extends CI_Controller{
        ...
        $boys_view = $this->classifed_model->boys_view($search_option);
        ...
        //$data["paging_links"] =  $this->pagination->create_links();
            ...
            $log_name = @mysql_result(mysql_query("SELECT first_name FROM `login` WHERE `login_id` = '$loginid' "), 0, 'first_name');
                ...
                $data['busconcount'] = $this->hotdealsearch_model->busconcount_boysview();
                /*service provided / needed for services*/
                $data['sellerneededcount'] = $this->hotdealsearch_model->sellerneeded_boyview();
                 /*packages count*/
                $data['deals_pck'] = $this->hotdealsearch_model->deals_pck_boysview();
        ...
        // $services_view = $this->classifed_model->services_view($search_option);
            ...
            /*location list*/
             $loc_list = $this->hotdealsearch_model->loc_list();
             $rs = $this->hotdealsearch_model->boys_view_search($search_option);
             if (!empty($rs)) {
                foreach ($rs as $sview) {
                        $loginid = $sview->login_id;
                    }
