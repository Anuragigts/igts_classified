...
class hotdealsearch_model extends CI_Model{
        ...
        public function clothstyle_search($data){
        	...
        	$women_list = $this->input->post('women_list');
        	$this->db->select("ad.*, img.*, COUNT(`img`.`ad_id`) AS img_count, loc.*");
			$this->db->select("DATE_FORMAT(STR_TO_DATE(ad.created_on,
	  		'%d-%m-%Y %H:%i:%s'), '%Y-%m-%d %H:%i:%s') as dtime", FALSE);
			// $this->db->from("postad AS ad");
			...
			if (!empty($women_list)) {
				$this->db->where_in('ad.sub_scat_id', $women_list);
			}
        ...
        public function count_men_view_search(){
        	...
        	$pck = $this->input->post('pckg_list');
        	$seller = $this->input->post('seller_deals');
        	$men_list = $this->input->post('men_list');
        	$this->db->select("ad.*, img.*, COUNT(`img`.`ad_id`) AS img_count, loc.*");
			$this->db->select("DATE_FORMAT(STR_TO_DATE(ad.created_on,
	  		'%d-%m-%Y %H:%i:%s'), '%Y-%m-%d %H:%i:%s') as dtime", FALSE);
			$this->db->from("postad AS ad");
			...
			if (!empty($men_list)) {
				$this->db->where_in('ad.sub_scat_id', $men_list);
			}
        ...
        public function count_boys_view_search(){
        	...
        	$pck = $this->input->post('pckg_list');
        	$seller = $this->input->post('seller_deals');
        	$boys_list = $this->input->post('boys_list');
        	$this->db->select("ad.*, img.*, COUNT(`img`.`ad_id`) AS img_count, loc.*");
			$this->db->select("DATE_FORMAT(STR_TO_DATE(ad.created_on,
	  		'%d-%m-%Y %H:%i:%s'), '%Y-%m-%d %H:%i:%s') as dtime", FALSE);
			$this->db->from("postad AS ad");
        ...
        public function count_girls_view_search(){
        	...
        	$pck = $this->input->post('pckg_list');
        	$seller = $this->input->post('seller_deals');
        	$girls_list = $this->input->post('girls_list');
        	$this->db->select("ad.*, img.*, COUNT(`img`.`ad_id`) AS img_count, loc.*");
			$this->db->select("DATE_FORMAT(STR_TO_DATE(ad.created_on,
	  		'%d-%m-%Y %H:%i:%s'), '%Y-%m-%d %H:%i:%s') as dtime", FALSE);
			$this->db->from("postad AS ad");
...
(SELECT COUNT(*) FROM postad AS ad WHERE ad.category_id = 'findaproperty' AND ad.sub_cat_id = 26) AS commercial");
        ...
        public function deals_pck_services(){
			...
			(SELECT COUNT(*) FROM postad WHERE category_id = 'services' AND package_type = 'platinum' AND urgent_package = '') AS platinumcount,
			(SELECT COUNT(*) FROM postad WHERE category_id = 'services' AND package_type = 'gold' AND urgent_package = '') AS goldcount,
			(SELECT COUNT(*) FROM postad WHERE category_id = 'services' AND package_type = 'free' AND urgent_package = '') AS freecount");
        	$rs = $this->db->get();
        	return $rs->result();
        }
