				...
				<form class="form-horizontal" id='blogform' action='' method='post' enctype="multipart/form-data">
					<fieldset>
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="blog_title" name="blog_title" value='<?php echo $getdata->blog_title; ?>'>
								<input type="hidden" id="blog_id" name="blog_id" value='<?php echo $getdata->id; ?>'>
							</div>
						</div>
						<div class="control-group">
							<label class="control-label" for="blog_desc">Blog Description</label>
							<div class="controls">
								<textarea id="blog_desc" name="blog_desc" rows='10' cols='80'><?php echo $getdata->blog_desc; ?></textarea>
							</div>
						</div>
						<div class="control-group">
							...
							<div class="controls">
								<select name='blog_cat' id="blog_cat">
									...
									<?php foreach ($allcategory as $catval) {	?>
										<option value="<?php echo $catval->category_id; ?>" <?php if ($getdata->blog_cat == $catval->category_id) { echo "selected='selected'"; }  ?> ><?php echo $catval->category_name; ?></option>
									<?php } ?>
								</select>
							</div>
						</div>
						<div class="control-group">
							...
							<div class="span4 unit img_hide top_10">
								<img id="blah" src="<?php echo base_url();?>pictures/blogs/<?php echo $getdata->blog_image; ?>"  alt=''/>
								<input type="hidden" name="imgname" id="imgname" value="<?php echo $getdata->blog_image; ?>" />
							</div>
						</div>
