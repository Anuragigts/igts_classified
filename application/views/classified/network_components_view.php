...
<title>Hub | Switch | Bridges | Routers | Brouters For Sale In UK | 99 Right Deals</title>

<meta name="description" content="Buy & Sell New & Used or Second Hand network component for sale in London, United Kingdom at the affordable price on 99 Right Deals." />
...
$network_sub = $this->session->userdata('network_sub');
...
<form id="j-forms2" action="<?php echo base_url(); ?>network_components_view/search_filters" method='post' class="j-forms jforms" style="background-color: rgb(255, 255, 255) !important;">
...
<div id='limit_scrol'>
<?php foreach ($networkssub as $networkssubval) { ?>
<label class="checkbox">
<input type="checkbox" name="network_sub[]" class="network_sub" value="<?php echo $networkssubval->sub_subcategory_id; ?>" <?php if (isset($network_sub) && in_array($networkssubval->sub_subcategory_id, $network_sub)) { echo "checked = checked";	} ?> >
<i></i> <?php echo $networkssubval->sub_subcategory_name; ?> (<?php echo $networkssubval->no_ads; ?>)
</label>
<?php } ?>
</div>
</div>
...
<?php echo $this->load->view("classified/network_components_view _search"); ?>
</div>
</div>
</div>
</div>
